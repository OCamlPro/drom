...
let action ~package_name ~kind ~mode ~promote_skip ~dir ~create ~remove =
  match Project.find_project () with
  | None ->
      Error.raise
        "No project detected. Maybe you want to use 'drom project --new \
         PROJECT' instead"
  | Some (p, inferred_dir) ->
      let name =
          ...
          List.iter
            (fun package ->
              if package.name = name then (
                ( match kind with
                | None -> ()
                | Some kind -> p.package.kind <- kind );
                match mode with
                | None -> ()
                | Some mode -> p.package.p_mode <- Some mode ))
            p.packages;
        Update.update_files ~create:false ?mode ~upgrade:true ~promote_skip
          ~git:true p )
