...
let action ~args cmd =
  let args =
    ...
    | None -> "ocaml" :: cmd
    | Some _ ->
        let (_p : Types.project) = Build.build ~dev_deps:true
            ~extra_packages:[ "utop" ] ~args () in
        let init_file = Filename.temp_file "ocaml" ".init" in
        let stdout = Unix.openfile init_file
            [ Unix.O_CREAT ; Unix.O_WRONLY ] 0o644 in
        Misc.call ~stdout [| "opam" ; "exec" ; "--" ; "dune" ; "top" |];
        Unix.close stdout;
        "utop" :: "-init" :: init_file :: cmd
  in
  let utop_history = Globals.home_dir // ".utop-history" in
  if not ( Sys.file_exists utop_history ) then
    begin
