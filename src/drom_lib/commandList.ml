...
let cmd =
  ...
  EZCMD.sub cmd_name
       ...
       List.iter (function
           | "projects" ->
               Printf.printf "Known project skeletons: %s\n%!"
                 ( String.concat " " (List.map (fun s -> s.skeleton_name)
    ...
    ~args: [
      ...
      EZCMD.info "Use 'projects' or 'packages' to display corresponding skeletons"
    ]
    ~doc:"List available project or packages skeletons"
    ~version:"0.4.0"
    ~man: [
      `S "DESCRIPTION";
      `Blocks [
        `P "List available project or packages skeletons"
      ]
