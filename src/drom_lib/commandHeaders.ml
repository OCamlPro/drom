...
let reset_headers env =
  List.iter
    (fun sep -> List.iter (fun h -> h.header_files <- []) sep.sep_headers)
...
let new_header env header_sep header_pos header_lines =
  ...
  if StringSet.mem header_id env.config.ignore_headers then
    ...
    let h =
      try Hashtbl.find env.headers header_id with
      | Not_found ->
        let h = { header_sep; header_id; header_lines; header_files = [] } in
...
let read_headers env lines header_sep =
  ...
  and iter_in pos lines header_pos header_lines headers =
    ...
    | line :: lines ->
      ...
      else if pos - header_pos > !max_header_lines then
        (* not a header *)
        iter_out (pos + 1) lines headers
      else
...
let record_header ?(config = false) env file_name header_sep =
  ...
  let file = { file_name; file_headers } in
  Hashtbl.add env.files file_name file;
  let file_headers =
    match file_headers with
    | [] ->
  ...
  List.fold_left
    (fun env file ->
      if file = "" then
        ...
        { env with ignore_files = StringSet.add file env.ignore_files } )
    env list

let list_ignore_headers env list =
  List.fold_left
    (fun env line ->
      { env with ignore_headers = StringSet.add line env.ignore_headers } )
    env list
  ...
  Array.iter
    (fun file ->
      ...
      | _ -> (
        ...
        with
        | None -> ()
        | Some false -> check_file env file dirfile
        | Some true ->
          if Sys.file_exists (Filename.concat dirfile ".ocp-check-headers-stop")
          then
            ()
          else
    ...
    | _ -> (
      if not (StringSet.mem ext env.config.ignore_extensions) then
        ...
        | _ ->
          if
            not
              ( StringSet.mem lfile env.config.ignore_files
  ...
  let rec insert_header pos lines rev_lines =
    if pos = line_pos then
      ...
      | [] ->
        ...
        raise Not_found
      | line :: lines -> insert_header (pos + 1) lines (line :: rev_lines)
  and check_src_header lines header_lines rev_lines =
    match (lines, header_lines) with
    | _, [] ->
      List.rev rev_lines @ dst_header.header_lines
...
let add_default_header header file =
  ...
  let rec insert_header pos lines rev_lines =
    if pos = sep.sep_add_line then
      List.rev rev_lines @ header.header_lines @ ("" :: remove_empty_lines lines)
    else
      match lines with
        ...
        try
          ...
          try
            ...
            List.iter
              (fun (_, file) ->
                if add_default_header header file then begin
                  Printf.fprintf (get_undo_oc ()) "add:%s:%s\n" header_id
                    file.file_name;
                  incr updates
                end )
            ...
            if !updates > 0 then begin
              Printf.printf "Scanning again after %d changes for %s\n%!"
                !updates header_id;
              let dirs = List.rev args.arg_dirs in
              scan_dirs env dirs
            end
  ...
  if args.arg_replace <> [] then
    List.iter
      (fun header_pair ->
        ...
        in
        let src_header =
          try Hashtbl.find env.headers src_id with
          | Not_found ->
        ...
        let dst_header =
          try Hashtbl.find env.headers dst_id with
          | Not_found ->
        ...
        List.iter
          (fun (line_pos, file) ->
            if replace_header src_header dst_header line_pos file then begin
              incr updates;
              Printf.fprintf (get_undo_oc ()) "replace:%s:%d:%s:%s\n" src_id
                line_pos dst_id file.file_name
            end )
...
let cmd =
  ...
  EZCMD.sub cmd_name
    ...
    ~args:
      [ ( [ "add-default" ],
          Arg.String (fun s -> args.arg_add_default <- s :: args.arg_add_default),
          EZCMD.info ~docv:"HEADER_ID"
            "Add this header as the default for these files" );
        ( [ "replace" ],
          Arg.String (fun s -> args.arg_replace <- s :: args.arg_replace),
          EZCMD.info ~docv:"SRC:DST" "Replace header SRC by header DST" );
        ( [ "replace-by" ],
          Arg.String (fun s -> args.arg_replace_by <- Some s),
