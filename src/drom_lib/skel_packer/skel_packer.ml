...
let () =
  ...
  | _exe :: kind :: name :: super :: skeleton_dir :: _deps ->
    ...
    let print_files files =
      ...
      begin
        match toml with
        | None -> Printf.printf "  skeleton_toml = [] ;\n"
        | Some content -> Printf.printf "  skeleton_toml = [ %S ] ;\n" content
      end;
    ...
    Array.iter
      (fun dir ->
        ...
        Printf.printf "  let header = [ %s ]\n"
          (String.concat "; \n"
             (List.map
                (fun line -> Printf.sprintf "  %S" line)
                (EzFile.read_lines (dirname // "HEADER") |> Array.to_list)));
