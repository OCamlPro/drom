...
(** The only exception that may be raised by substitutions: it indicates that
    the end of the expression could not be found. *)
exception UnclosedExpression of string

(** [string f context s] performs substitutions on [s] following [f], passing
    the context [context] to [f] for every expression, returning the result as a
    string. *)
val string :
  ?sep:char ->
  ?sym:bool ->
...
(** [buffer f b context s] performs substitutions on [s] following [f], passing
    the context [context] to [f] for every expression, returning the result by
    appending it to the buffer [b]. *)
val buffer :
  ?sep:char ->
  ?sym:bool ->
  ?escape:bool ref ->
