    ...
    if all then begin
      ...
      Array.iter (fun switch ->
          ...
          | '3'..'9' when not ( String.contains switch '+') ->
              if switch >= p.min_edition &&
                 Sys.file_exists ( root // switch // ".opam-switch" ) then begin
