  ...
  let upgrade =
    Hashes.with_ctxt ~git:true (fun hashes ->
        ...
        | Some name ->
            ...
            if List.for_all (fun package -> package.name <> name)
                p.packages then
        ...
        | None ->
            let upgrade =
              match create with
              | Some skeleton ->
                  if List.exists (fun package -> package.name = name)
                      p.packages then
                    Error.raise "A package with this name already exists";
                  let dir =
                    match dir with
                    | None ->
                        let dir = if inferred_dir = "" then "src"
                          else inferred_dir in
                        dir // name
                    | Some dir -> dir
                  in
              ...
              | None ->
                  ...
                  | Some new_name ->
                      ...
                      if List.exists (fun package ->
                          package.name = new_name) p.packages
                      then
                        Error.raise
